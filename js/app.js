	...
	function loadData(url){
		...
		.then((out) => {
			...
			if(!use_php_api && !API_KEY){
				feedbackError("❌ Erro: a chave de API não foi definida","error");
			}
				  ...
				  <div class="swiper-slide">
						<div class="character-card">
							...
							<div class="character-button">
								<span data-index="${i}" class='open-modal'>Falar com ${array_characters[i]['name']}</span>
							</div>
							</div>
						</div>				
				  </div>
	...
	return timestamp.toLocaleString();
		...
		async function getResponse(prompt) {
			...
			.then(data => {
			  ...
			  if(continuous_chat){
				  pmt = `O que se segue é uma conversa entre ${character_name}: e usuário: \n\n ${conversations}`;
			  }else{
				  pmt = prompt;
			  }			  
		...
		function apiError(error){
			feedbackError("❌ Erro, tente novamente","error")
			scrollChatBottom();	
			enableChat();
			if (!(/Mobi/.test(navigator.userAgent))) {
				$("#chat").focus();
		...
		function sendUserChat(){
			...
			if(chat.length < chat_minlength){
				feedbackError(`❌ Insira uma mensagem com mais de ${chat_minlength} caracteres`,"error");
				return false;
			}

			if(chat == ""){
				feedbackError("❌ Digite uma mensagem","error");
				$("#chat").focus();
				return false;
			}
